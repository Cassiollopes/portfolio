  ...
  return (
    ...
    >
      <Link
        href={link}
        target="_blank"
        className="flex md:min-h-[200px] relative overflow-hidden w-full justify-center shadow-inner"
      >
        <Image
          src={images[0]}
          alt="Portfolio"
      ...
      <div className="h-full w-full flex pt-2 p-5 justify-start flex-col gap-3">
        ...
        <div className="flex flex-wrap gap-1.5 pb-1 mt-auto">
          {technologies.map((tech, i) => (
            <div
              key={i}
              className="p-0.5 px-2 dark:bg-white/5 bg-black/5 rounded-md flex gap-1 items-center font-semibold text-[10px] text-foreground/90"
            >
              {tech.icon}
              {tech.label}
            </div>
